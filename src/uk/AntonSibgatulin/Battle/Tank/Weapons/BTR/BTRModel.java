package uk.AntonSibgatulin.Battle.Tank.Weapons.BTR;
...
public class BTRModel extends Tanks implements ITank{
...
public String toString(){
	...
	public BTRModel(String name,int maxShot,int minShot,long reload,int maxsplash,int minsplash,int speed,long maxDistance,Hull hull) {
	...
	public void fire(String str) {
		if(System.currentTimeMillis()-lastTime >=this.getTimeReload()){
			lastTime = System.currentTimeMillis();
			
			Pos pos = this.tank.pos;
			double rnd = this.minSplash+new Random().nextInt((int) (maxSplash-minSplash+1));
			double recx = 0;
			double recy = 0;
			...
			field.send_to_all("table;shot;create;"+id+";"+(int)vec.x+";"+(int)vec.y+";"+(int)rnd, null);
			while(true){
				try {
			...
			System.err.println("Time reload..." +(this.getTimeReload()-(System.currentTimeMillis()-lastTime))+"ms");
		}
	}
	@Override
	...
	public void setField(BattleField field) {
	 ...
	 public Object clone()
        ...
        {
          return (ITank)new BTRModel( type, maxShot, minShot, reload, maxSplash, minSplash, (int)speed, (long)maxDistance, hull);
        }
	@Override
